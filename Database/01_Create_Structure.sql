...
(size INT) RETURNS TEXT AS $$
DECLARE
  characters TEXT := 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
  bytes BYTEA := gen_random_bytes
(size);
  l INT := length
(characters);
  i INT := 0;
...
(
  ...
  auth_code CHAR(6),
  resp_code CHAR(5) REFERENCES RESPONSECODES(Code),
  processed_on TIMESTAMP DEFAULT NOW() NOT NULL,
  reference VARCHAR,
  source_id TEXT REFERENCES PAYMENT_SOURCES(id),
  CONSTRAINT valid_status CHECK (payment_status in ('Captured', 'Authorized', 'Declined', 'Failed'))
);
